...
class Hall(models.Model):
    ...
    HALL_TYPES = [
        ('AC', 'AC'),
        ('Open Roof', 'Open Roof'),
        ('Close Roof', 'Close Roof')
    ]

    name = models.CharField(max_length=30)
    hall_type = models.CharField(max_length=10, choices=HALL_TYPES)
...
class Show(models.Model):
    ...
    def __str__(self):
        return f"[{self.concert}] - [ {self.get_time_display()} | {self.date.strftime('%d %B, %Y')} ]"
...
def createTheater(sender, instance, **kwargs):
    hall_types = [{'AC': 'Hall 1'}, {'Open Roof': 'Hall 2'},
                  {'Close Roof': 'Hall 3'}]
